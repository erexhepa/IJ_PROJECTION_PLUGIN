 ...
 */
public class MIP_KMeansFrame extends JFrame
implements ActionListener, MIP_KMeansListener {
    ...
    private MIP_Cluster[] clustersKmeans;
    ...
    public MIP_Cluster[] getClustersKmeans() {
        return clustersKmeans;
    }

    public void setClustersKmeans(MIP_Cluster[] clustersKmeans) {
        this.clustersKmeans = clustersKmeans;
    }

    public MIP_KMeansFrame(int numClust_, double[][] result_fft) {
    ...
    throws MIP_InsufficientMemoryException {
...

        {
            ...
            try {
                ...
                if (implementation == BASIC_KMEANS) {
                    mKMeans = new MIP_BasicKMeans(coordinates, clusterCount, 500, randomSeed);
                } else if (implementation == BENCHMARKED_KMEANS) {
                    mKMeans = new MIP_BenchmarkedKMeans(coordinates, clusterCount, 500, randomSeed);
                } else if (implementation == CONCURRENT_KMEANS) {
                    try {
                        int threadCount = (int) getEnteredValue(mThreadCountTF, 1L, 20L);
                        mKMeans = new MIP_ConcurrentKMeans(coordinates, clusterCount, 500, randomSeed, threadCount);
    ...
    public void kmeansComplete(MIP_Cluster[] clusters, long executionTime) {
    ...
    public void kmeansError(Throwable err) {
        cleanupAfterKMeans();
        if (err instanceof MIP_InsufficientMemoryException) {
