 ...
 */
public class MIP_ConcurrentKMeans implements MIP_KMeans {
    ...
    private MIP_Cluster[] mClusters;
    private MIP_Cluster[] mClusters2;
    ...
    private List<MIP_KMeansListener> mListeners = new ArrayList<MIP_KMeansListener>(1);
     ...
     */
    public MIP_ConcurrentKMeans(double[][] coordinates, int k, int maxIterations,
    ...
    public MIP_Cluster[] getmClusters2() {
        return mClusters2;
    }

    public void setmClusters2(MIP_Cluster[] mClusters2) {
     ...
     */
    private void postKMeansComplete(MIP_Cluster[] clusters, long executionTime) {
        if (mListeners.size() > 0) {
            synchronized (mListeners) {
                int sz = mListeners.size();
                for (int i=0; i<sz; i++) {
     ...
     */
    private void computeDistances() throws MIP_InsufficientMemoryException {
        ...
        if (mDistanceCache == null) {
            ...
            if (Runtime.getRuntime().freeMemory() < memRequired) {
                throw new MIP_InsufficientMemoryException();
            }
     ...
     */
    private MIP_Cluster[] generateFinalClusters() {
        ...
        List<MIP_Cluster> clusterList = new ArrayList<MIP_Cluster>(numClusters);
        for (int c = 0; c < numClusters; c++) {
            ProtoCluster pcluster = mProtoClusters[c];
            if (!pcluster.isEmpty()) {
                MIP_Cluster cluster = new MIP_Cluster(pcluster.getMembership(), pcluster.getCenter());
                clusterList.add(cluster);
            }
        }
    
        // - convert list to an array.
        MIP_Cluster[] clusters = new MIP_Cluster[clusterList.size()];
