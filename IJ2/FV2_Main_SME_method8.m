function [OUTput_image,Manifold,Classmap,idmaxini,cost,WW,sftzm,dekay]=Main_SME_method(Img)
...
%% Finding step soze and effective stacks
%%
KE= max(idmax(edgeflag2>0))- min(idmax(edgeflag2>0))+1;
step=KE/100;

%% Finding the W parameter
%%
...
totalz=sum(timk(:));
%      Img(:,:,imageSize(3)+1)=zeros(size(Img,1),size(Img,2));
sftz=[];
for k=1:size(A,3)
sft=k-1;
qzr3=qzr2-sft;
